...
static int ora_seq_prefix_search(const char *name, char * array[], int max);
...
static int
ora_seq_prefix_search(const char *name, char * array[], int max)
{
	int		i;
...
ora_session_timezone(PG_FUNCTION_ARGS)
{
	...
	if (HasCTZSet)
	{
		...
		resinterval->time = -(CTimeZone * USECS_PER_SEC);
#else
		resinterval->time = -CTimeZone;
#endif
		PG_RETURN_INTERVAL_P(resinterval);
	}
