...
static text *random_string(const char *charset, int chrset_size, int len);
...
{
	int seed = PG_GETARG_INT32_0_IF_NULL(0);
...
{
	text *key = PG_GETARG_TEXT_P_IF_NULL(0);
	Datum seed;
...
dbms_random_string(PG_FUNCTION_ARGS)
{
	...
	if (txt == NULL)
		...
		option = text_to_cstring(txt)[0];
	len = PG_GETARG_INT32_0_IF_NULL(1);
...
dbms_random_value_range(PG_FUNCTION_ARGS)
{
	...
	if (low > high)
		result = ((double) rand() / ((double) RAND_MAX + 1)) * (low - high) + high;
	else
