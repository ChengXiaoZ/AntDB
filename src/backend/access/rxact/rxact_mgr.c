...
static void appendStringOids(StringInfo buf, int count, Oid *oid)
{
	char tmp = '{';
	int i = 0;
	AssertArg(buf && oid);
...
void rlog_desc(const char *file_name)
{
	RXactLog rlog;
	Oid *oids;
	const char *gid;
	...
	if(file_name == NULL)
	{
		fprintf(stderr, _("please specify a file\n"));
		proc_exit(1);
	}
	...
	rfile = PathNameOpenFile(pstrdup(file_name), O_RDONLY | PG_BINARY, 0);
	if(rfile < 0)
	{
		ereport(FATAL,
			(errcode_for_file_access(),
			errmsg("could not open%s file \"%s\":%m", "", file_name)));
	}
	rlog = rxact_begin_read_log(rfile);
	initStringInfo(&buf);
	...
	for(;;)
	{
		...
		if(c == RXACT_MSG_DO)
		{
			...
			appendStringOids(&buf, count, oids);
			...
			|| c == RXACT_MSG_FAILED)
		{
			...
			appendStringInfo(&buf, "%s %s %s"
				, gid, c == RXACT_MSG_SUCCESS ? "success":"failed"
				, RemoteXactType2String(type));
		}else if(c == RXACT_MSG_CHANGE)
		{
			type = (RemoteXactType)rxact_log_get_int(rlog);
			gid = rxact_log_get_string(rlog);
			appendStringInfo(&buf, "%s change %s", gid, RemoteXactType2String(type));
		}else
		{
			rxact_report_log_error(rfile, FATAL);
		}
