...
for dir in $INCLUDE_DIRS; do
    if [ -f "$dir/catalog/pg_namespace.h" ]; then
        ...
        PG_MANAGER_NAMESPACE=`grep '^#define[ 	]*PG_MANAGER_NAMESPACE' $dir/catalog/pg_namespace.h | $AWK '{ print $3 }'`
        break
    fi
done
...
sed -e "s/;[ 	]*$//g" \
    ...
    -e "s/ORANSP/$PG_ORACLE_NAMESPACE/g" \
    -e "s/MGRNSP/$PG_MANAGER_NAMESPACE/g" \
| $AWK '
# ----------------
