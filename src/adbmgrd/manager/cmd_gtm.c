...
void mgr_add_gtm(MGRAddGtm *node, ParamListInfo params, DestReceiver *dest)
{
	...
	foreach(lc,node->options)
	{
		...
		}else if(strcmp(def->defname, "type") == 0)
		{
			...
			else if(strcmp(str, "gtm_proxy") == 0)
				datum[Anum_mgr_gtm_gtmtype-1] = CharGetDatum(GTM_TYPE_PROXY);
			else
				ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR)
					, errmsg("gtm \"%s\" not recognized", str)
					, errhint("option type is gtm  gtm_standby or gtm_proxy")));
...
/*
* execute init gtm master/slave, send infomation to agent to init gtm master 
*/
Datum 
mgr_init_gtm(PG_FUNCTION_ARGS)
{
	...
	cmdtype = AGT_CMD_GTM_INIT;
	gtmname = PG_GETARG_CSTRING(0);
	if (strcmp(gtmname, "master") == 0)
	{
		gtmtype = GTM_TYPE_GTM;
	}
	else if (strcmp(gtmname, "slave") == 0)
	{
		gtmtype = GTM_TYPE_STANDBY;
	}
	else
	{
		/*never come here*/
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		mgr_init_gtm_kind(cmdtype, &getAgentCmdRst, rel_gtm, aimtuple);
...
/*
*	init gtm_proxy proxy1,proxy2......
* init gtm_proxy all
*/
Datum 
mgr_init_gtm_proxy(PG_FUNCTION_ARGS)
{
	List *nodenamelist;
	GetAgentCmdRst getAgentCmdRst;
	...
	Form_mgr_gtm mgr_gtm;
	ScanKeyData key[1];
	const char nodetype = GTM_TYPE_PROXY;
	const char cmdtype = AGT_CMD_GTM_PROXY_INIT;
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		info->rel_gtm = heap_open(GtmRelationId, RowExclusiveLock);
		if(PG_ARGISNULL(0)) /* no argument, start all */
		{
			/*add all the type of gtm name to list*/
			ScanKeyInit(&key[0],
				Anum_mgr_gtm_gtmtype
				,BTEqualStrategyNumber
				,F_CHAREQ
				,CharGetDatum(nodetype));
			info->rel_scan = heap_beginscan(info->rel_gtm, SnapshotNow, 1, key);
			while((tuple = heap_getnext(info->rel_scan, ForwardScanDirection)) != NULL)
			{
					mgr_gtm = (Form_mgr_gtm)GETSTRUCT(tuple);
					Assert(mgr_gtm);
					nodename = NameStr(mgr_gtm->gtmname);
	...
	if (*lcp == NULL)
	{
		heap_close(info->rel_gtm, RowExclusiveLock);
		SRF_RETURN_DONE(funcctx);
	}
	nodename = (char *) lfirst(*lcp);
	*lcp = lnext(*lcp);
	aimtuple = SearchSysCache1(GTMGTMNAME, CStringGetDatum(nodename));
	if (!HeapTupleIsValid(aimtuple))
		elog(ERROR, "cache lookup failed for relation %s", nodename);
	/*get execute cmd result from agent*/
	mgr_init_gtm_kind(cmdtype, &getAgentCmdRst, info->rel_gtm, aimtuple);
	tup_result = build_common_command_tuple(
		&(getAgentCmdRst.nodename)
		, getAgentCmdRst.ret
		, getAgentCmdRst.description.data);
	ReleaseSysCache(aimtuple);
	pfree(getAgentCmdRst.description.data);
	SRF_RETURN_NEXT(funcctx, HeapTupleGetDatum(tup_result));
}

void 
mgr_init_gtm_kind(const char cmdtype, GetAgentCmdRst *getAgentCmdRst, Relation gtmrel, HeapTuple aimtuple)
{
	...
	pfree(hostaddress);
	/*check gtm master init or not*/
	if (mgr_gtm->gtminited)
	{
	...
	appendStringInfo(&infosendmsg, " -D %s", gtmPath);
	if(GTM_TYPE_GTM == gtmtype || GTM_TYPE_STANDBY == gtmtype)
		ma_sendstring(&infosendmsg," -Z gtm");
	else if(GTM_TYPE_PROXY)
		ma_sendstring(&infosendmsg," -Z gtm_proxy");
	/* send message */
  ...
  initdone = mgr_recv_msg(ma, getAgentCmdRst);
	...
	/*update gtm system table's column to set initial is true*/
	if (initdone)
	{
...
mgr_init_gtm_all(PG_FUNCTION_ARGS)
{
	...
	{
		mgr_init_gtm_kind(AGT_CMD_GTM_INIT, &getAgentCmdRst, info->rel_gtm, tuple);
		tup_result = build_common_command_tuple(
			&(getAgentCmdRst.nodename)
			, getAgentCmdRst.ret
			, getAgentCmdRst.description.data);
	...
	else if (mgr_gtm->gtmtype == GTM_TYPE_STANDBY)
	{
		mgr_init_gtm_kind(AGT_CMD_GTM_INIT, &getAgentCmdRst, info->rel_gtm, tuple);
		tup_result = build_common_command_tuple(
			&(getAgentCmdRst.nodename)
	...
	else /* CharGetDatum(datumType) == GTM_TYPE_PROXY */
	{
		mgr_init_gtm_kind(AGT_CMD_GTM_PROXY_INIT, &getAgentCmdRst, info->rel_gtm, tuple);
		tup_result = build_common_command_tuple(
			&(getAgentCmdRst.nodename)
			, getAgentCmdRst.ret
			, getAgentCmdRst.description.data);
...
Datum mgr_start_gtm(PG_FUNCTION_ARGS)
{
	...
	const char *gtmname = PG_GETARG_CSTRING(0);
	/*check the name is master or slave*/
	if(strcmp(gtmname, "master")!=0 && strcmp(gtmname, "slave")!=0)
	{
		ereport(ERROR, (errcode(ERRCODE_DATA_EXCEPTION)
			, errmsg("it should start gtm master or start gtm slave")));
	}
	if (strcmp(gtmname, "master") == 0)
	{
		gtmtype = GTM_TYPE_GTM;
		cmdtype = AGT_CMD_GTM_START_MASTER;
	}
	else if(strcmp(gtmname, "slave") == 0)
	{
		gtmtype = GTM_TYPE_STANDBY;
		cmdtype = AGT_CMD_GTM_START_SLAVE;
	}
	else
	{
			/*never come here*/
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		mgr_runmode_gtm_get_result(cmdtype, &getAgentCmdRst, rel_gtm, aimtuple);
		tuple = build_common_command_tuple(
			&(getAgentCmdRst.nodename)
			, getAgentCmdRst.ret
			, getAgentCmdRst.description.data);
...
void 
mgr_runmode_gtm_get_result(const char cmdtype, GetAgentCmdRst *getAgentCmdRst, Relation gtmrel, HeapTuple aimtuple)
{
	/*get gtm path from adbmgr.gtm, which row info type is gtm*/
	Datum datumPath;
	char *gtmPath;
	...
	appendStringInfo(&infosendmsg, " -Z %s ", zmode);
	ma_sendstring(&infosendmsg," -l logfile");
	/* send message */
	ma_beginmessage(&buf, AGT_MSG_COMMAND);
	ma_sendbyte(&buf, cmdtype);
	ma_sendstring(&buf,infosendmsg.data);
...
Datum mgr_stop_gtm(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		mgr_runmode_gtm_get_result(AGT_CMD_GTM_STOP_MASTER, &getAgentCmdRst, rel_gtm, aimtuple);
		tuple = build_common_command_tuple(
			&(getAgentCmdRst.nodename)
			, getAgentCmdRst.ret
			, getAgentCmdRst.description.data);
...
/*
* start gtm_proxy gtm_proxy1,gtm_proxy2....
* start gtm_proxy all
*/
Datum mgr_start_gtm_proxy(PG_FUNCTION_ARGS)
{
	List *nodenamelist;
	GetAgentCmdRst getAgentCmdRst;
	HeapTuple tup_result
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		if(PG_ARGISNULL(0)) /* no argument, start all */
		{
			/*add all gtm_proxy name to list*/
			ScanKeyInit(&key[0],
				Anum_mgr_gtm_gtmtype
				,BTEqualStrategyNumber
				,F_CHAREQ
				,CharGetDatum(GTM_TYPE_PROXY));
			info->rel_scan = heap_beginscan(info->rel_gtm, SnapshotNow, 1, key);
			while((tuple = heap_getnext(info->rel_scan, ForwardScanDirection)) != NULL)
			{
					mgr_gtm = (Form_mgr_gtm)GETSTRUCT(tuple);
	...
	*lcp = lnext(*lcp);
	...
	if (!HeapTupleIsValid(aimtuple))
		elog(ERROR, "cache lookup failed for relation %s", nodename);
	/*get execute cmd result from agent*/
	mgr_runmode_gtm_get_result(AGT_CMD_GTM_START_PROXY, &getAgentCmdRst, info->rel_gtm, aimtuple);
	tup_result = build_common_command_tuple(
		&(getAgentCmdRst.nodename)
		, getAgentCmdRst.ret
		, getAgentCmdRst.description.data);
...
/*
* stop gtm_proxy gtm_proxy1,gtm_proxy2....
* stop gtm all
*/
Datum mgr_stop_gtm_proxy(PG_FUNCTION_ARGS)
{
	List *nodenamelist;
	GetAgentCmdRst getAgentCmdRst;
	HeapTuple tup_result
	...
	*lcp = lnext(*lcp);
	...
	if (!HeapTupleIsValid(aimtuple))
		elog(ERROR, "cache lookup failed for relation %s", nodename);
	/*get execute cmd result from agent*/
	mgr_runmode_gtm_get_result(AGT_CMD_GTM_STOP_PROXY, &getAgentCmdRst, info->rel_gtm, aimtuple);
	tup_result = build_common_command_tuple(
		&(getAgentCmdRst.nodename)
		, getAgentCmdRst.ret
		, getAgentCmdRst.description.data);
