...
#ifdef ADB
	printf("  -N, --nodename=NODE\n");
	printf("                 set current node's name for ADB\n");
#endif
...
#ifdef ADB
	while ((c = getopt_long(argc, argv, "D:N:vn", long_options, &option_index)) != -1)
#else
	...
	*/
	...
	else
	{
		...
		else
		{
			...
			fprintf(stderr, "Invalid control file");
			exit(1);
		}
